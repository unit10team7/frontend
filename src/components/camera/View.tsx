...
const CameraView = forwardRef<Webcam, CameraViewProps>(function CameraView(
  { type, stepCount, url, timerInfo, layoutType },
  ref,
) {
...
const StepFontContainer = styled.div<{ layout: LayoutType }>`
  display: flex;
  ${(props) => (props.layout === "vertical" ? " justify-content: center;" : undefined)}
  align-items: flex-start;
  position: absolute;
  top: 0;
  right: 0;
  bottom: 0;
  ${(props) => (props.layout === "vertical" ? "left: 0;" : "left: 145px;")}
  margin: auto;
  z-index: 1;
  padding-top: 37px;
`;
...
const StepFont = styled.span<{ layout: LayoutType }>`
  ...
  font-size: 22px;
  text-align: center;
  ${(props) => (props.layout === "vertical" ? undefined : "transform: rotate(90deg)")}
`;
...
const PhotoContainer = styled.div<{ layout: LayoutType }>`
  ...
  position: absolute;
  ${(props) => (props.layout === "vertical" ? "bottom: 0;" : "top: 0;")}
  z-index: 1;
`;
...
const PhotoCard = styled.img<{ layout: LayoutType }>`
  ...
  width: 124px;
  height: 180px;
  ${(props) => (props.layout === "vertical" ? "margin-bottom: 22px;" : "margin-top: 22px;")}
  margin-left: 14px;
  max-width: 100%;
  height: auto;
`;
